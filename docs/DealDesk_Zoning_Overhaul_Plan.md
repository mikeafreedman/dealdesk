...
Short format: `{deal_id}_{scenario_id}_financial_model.xlsx` (Session 4 kickoff Decision A; supersedes the speculative `{address_slug}_S{rank}_{descriptor}.xlsx` format from the original master plan, which was written pre-Session-1 before the schema landed and the `deal_id` / `scenario_id` slugs became canonical).

The `scenario_id` is generated upstream as a snake_case slug rooted in scenario semantics, validated against regex `^[a-z][a-z0-9_]*$` with a 30-char cap (per the `validate_scenario_id_format` field validator on `DevelopmentScenario` in `models/models.py`).

Examples for the Belmont reference deal with 3 scenarios:
...
Plus one master index file per deal: `{deal_id}_scenarios_index.xlsx`.

PDF stays as `{address_slug}_Underwriting_Report.pdf` for now. PDF naming convention is intentionally deferred — Session 5 (rendering) will decide whether to retain the `address_slug` pattern (which carries more human-readable context for report folders) or align to the per-scenario `{deal_id}_{scenario_id}` pattern. Not realigned in this post-Session-4 docs patch.
...
### April 29, 2026 — Post-Session-4 docs realignment
- Pure docs patch — no code changes, no schema changes — closing the two docs-realignment carry-forwards from the Session 4 history-log entry above. Discovery scan surfaced three additional propagation sites beyond the two known items; all five resolved in a single Option-B realignment patch (mirrors the post-Session-3 realignment pattern where 8 propagated drift sites surfaced beyond the 3 originally flagged carry-forwards).
- Master plan D7 filename format realigned from speculative `{address_slug}_S{rank}_{descriptor}.xlsx` to the actual implementation `{deal_id}_{scenario_id}_financial_model.xlsx` per Session 4 kickoff Decision A. Three propagated examples in the D7 section (originally `967-73_N_9th_St_S1_AsBuilt_Reno_21u.xlsx` style) realigned to Belmont-style examples grounded in the actual `belmont_*` scenario_id slugs that ship in the reference fixture, rather than fabricated illustrations. Brief inline supersession note added to the D7 prescription explaining the original master plan was written pre-Session-1 before the schema landed.
- PDF naming reference at D7 (`{address_slug}_Underwriting_Report.pdf`) intentionally left untouched and explicitly deferred to Session 5 (rendering) — the human-readable `address_slug` pattern may genuinely be the right call for report folders even though it would be the wrong call for Excel models, and the PDF format question is a real design decision Session 5 should make explicitly when the rendering work happens. Framed as deliberate deferral, not oversight, both in the D7 section itself and in this history-log entry.
- Session 4 kickoff §1 Decision E mapping table: original broken `rent_delta_pct` row preserved verbatim with a `[CORRECTED post-CP2 — see note below]` marker, plus a blockquote correction note explaining that `monthly_rent` does not exist on `FinancialAssumptions` (removed in a prior refactor) and the actual post-CP2 implementation uses `snapshot.rent_multiplier = 1.0 + delta` against the new `rent_multiplier: float = 1.0` field added in CP2. Original prescription preserved per the kickoff-as-historical-artifact rule — future readers see both the design-time error and the implementation-time correction.
- Same preservation treatment applied to the kickoff's §CP2 pseudocode block (the `_run_financials_for_scenario` worker example), which propagated the same `monthly_rent *= ...` error in code form. Original buggy line preserved with a `[POST-CP2 CORRECTION — see note after this block]` marker, plus a blockquote correction note pointing back to the §1 Decision E rationale and naming the actual `_apply_scenario_deltas_to_assumptions(snapshot, scenario)` helper in `financials.py` where the correct implementation lives.
- Two carry-forwards from Session 3 remain OPEN and DEFERRED — neither was reopened by Session 4 nor closed by this realignment: #4 — `is_split_zoned: bool = False` and `split_zoning_codes: List[str] = Field(default_factory=list)` on the `Zoning` sub-model (defensive `getattr` reads always return defaults due to Pydantic v2 behavior); #5 — `investment_strategy: Optional[InvestmentStrategy]` on `DevelopmentScenario` for server-side validation against `workflow_controls.strategy_lock` (LLM compliance with `strategy_lock` not validated server-side).
- Commit: `c70ba93` — no tag (consistent with prior docs commits).
