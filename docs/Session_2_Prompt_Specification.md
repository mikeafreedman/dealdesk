...
| Failure | Fallback |
|---|---|
| Confidence gate fails | Fully-typed CONFORMITY_INDETERMINATE assessment per `market.py:_indeterminate_conformity_assessment` (the canonical source): `status` = `CONFORMITY_INDETERMINATE`; `confidence` = `INDETERMINATE` (the consistency validator on `ConformityAssessment` requires this pairing); `confidence_reasons` enumerates which of the four confidence-gate criteria failed (sourced from `_confidence_gate_reasons(deal)`); `nonconformity_details` = `[]`; `grandfathering_status` = `None`; `proposed_pathway_requirement` = `None`; `risk_summary` is sentinel prose noting the gate failure and that manual diligence is required before LOI; `diligence_actions_required` lists four concrete actions (retrieve municipal zoning ordinance, confirm lot SF from deed/survey, re-run after populating zoning fields, engage local zoning counsel). 3C-SCEN still runs against this INDETERMINATE state and produces the single fallback scenario flagged HIGH entitlement risk (see row 3). |
| 3C-CONF call fails (both attempts) | Same `_indeterminate_conformity_assessment` fallback as confidence-gate failure (above), with one distinction: `confidence_reasons` carries a parse-failure message ("3C-CONF prompt persistently failed JSON parsing after retry") instead of the gate-criteria enumeration. |
| 3C-SCEN call fails (both attempts) | Single "as-submitted" fallback scenario per `market.py:_fallback_as_submitted_scenario` (the canonical source): `scenario_id` = `"as_submitted"`; `rank` = `1`; `scenario_name` = `"As-Submitted (Fallback)"`; `verdict` = `PREFERRED`; `unit_count` and `building_sf` reconstructed from `deal.assumptions` (default 0 if missing); `use_mix` = `[]`; `business_thesis` is sentinel prose explaining that the scenario is a placeholder and manual review is required; `operating_strategy` is sentinel prose referencing the user-submitted strategy; `zoning_pathway.pathway_type` = `BY_RIGHT` (sentinel — verify with zoning counsel); `zoning_pathway.success_probability_pct` = `None`; all other `zoning_pathway` fields `None`; assumption deltas (`construction_budget_delta_usd`, `rent_delta_pct`, `timeline_delta_months`) all `None`; `key_risks` lists three sentinel risks (placeholder scenario, BY_RIGHT defaulted without analysis, no alternates produced); `entitlement_risk_flag.severity` = `"HIGH"`; `entitlement_risk_flag.risk_summary` opens with "Scenario generation failed; manual review required." and notes the BY_RIGHT pathway is unverified against the zoning code; `entitlement_risk_flag.diligence_required` lists three actions (re-run 3C-SCEN with cleaned inputs, engage zoning counsel to verify BY_RIGHT, generate alternate scenarios). |
